...
import (
	...
	"github.com/gitvibav/url-shortener-application/api/utils"
...
func ShortenURL(c *gin.Context) {
	...
	if err == redis.Nil {
		r2.Set(database.Ctx, c.ClientIP(), os.Getenv("API_QUOTA"), 30*60*time.Second).Err()
	} else {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Unable to connect to the redis source",
		})
		return
	}
